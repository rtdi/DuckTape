...
def process():
    ...
    source_table.show(duckdb)
    tc.execute(duckdb)
    print("CDC table after table_comparison()")
    output_table.show(duckdb)
    scd2.execute(duckdb)
    print("CDC table after scd2()")
    output_table.show(duckdb)
    gk.execute(duckdb)
    print("CDC table after generate_key()")
    output_table.show(duckdb)
    apply(output_table, target_table)
    target_table.show(duckdb)
